...
import { Bot, Mail, Github, LogIn } from 'lucide-react';
...
export default function LoginModal() {
  ...
  const handleJavaLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store token in localStorage for now (legacy compatibility)
      ...
      <DialogContent className="sm:max-w-md bg-navy-800 border-navy-700 text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-sand-400">
            Access your secure Java-powered AI workspace.
          </DialogDescription>
        </DialogHeader>

        <div className="flex flex-col space-y-4 py-4">
          {/* Social Logins (Supabase) */}
          <div className="grid grid-cols-2 gap-4">
            <Button
              onClick={handleGoogleLogin}
              disabled={isLoading}
              variant="outline"
              className="w-full bg-white text-navy-900 hover:bg-sand-100 border-none h-12 text-sm font-semibold flex items-center justify-center space-x-2"
            >
            ...
            <Button
              ...
              disabled={isLoading}
              variant="outline"
              className="w-full bg-navy-900 text-white hover:bg-navy-950 border border-navy-700 h-12 text-sm font-semibold flex items-center justify-center space-x-2"
            >
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-navy-800 px-2 text-sand-500">Or use Java Auth</span>
            </div>
          </div>
          ...
          <form onSubmit={handleJavaLogin} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full bg-warm-500 hover:bg-warm-600 text-white h-12 font-bold"
            >
